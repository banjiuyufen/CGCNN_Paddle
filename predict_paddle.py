...
if os.path.isfile(args.modelpath):
    ...
    model_checkpoint = paddle.load(args.modelpath,
...
def main():
    ...
    if os.path.isfile(args.modelpath):
        ...
        checkpoint = paddle.load(args.modelpath,
                                map_location=lambda storage, loc: storage)
...
def validate(val_loader, model, criterion, normalizer, test=False):
    ...
    for i, (input, target, batch_cif_ids) in enumerate(val_loader):
        with paddle.no_grad():
            if args.cuda:
                input_var = (
                            paddle.to_tensor(input[0], place=paddle.CUDAPlace(0), non_blocking=True),
                            paddle.to_tensor(input[1], place=paddle.CUDAPlace(0), non_blocking=True),
                            paddle.to_tensor(input[2], place=paddle.CUDAPlace(0), non_blocking=True),
                            [paddle.to_tensor(crys_idx, place=paddle.CUDAPlace(0), non_blocking=True) for crys_idx in input[3]])
            else:
                input_var = (paddle.to_tensor(input[0]),
                            paddle.to_tensor(input[1]),
                            input[2],
                            input[3])
        if model_args.task == 'regression':
