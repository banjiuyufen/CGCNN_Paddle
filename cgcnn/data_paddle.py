...
from paddle.io import SubsetRandomSampler, BatchSampler
...
def get_train_val_test_loader(dataset,collate_fn=default_collate,
    ...
    train_sampler = BatchSampler(train_sampler, batch_size=batch_size)
    
    val_sampler = SubsetRandomSampler(
        indices[-(valid_size + test_size):-test_size])
    val_sampler = BatchSampler(val_sampler, batch_size=batch_size)
    if return_test:
        test_sampler = SubsetRandomSampler(indices[-test_size:])
        test_sampler = BatchSampler(test_sampler, batch_size=batch_size)
...
def collate_pool(dataset_list):
    ...
    batch_atom_fea: paddle.Tensor shape (N, orig_atom_fea_len)
      Atom features from atom type
    batch_nbr_fea: paddle.Tensor shape (N, M, nbr_fea_len)
      Bond features of each atom's M neighbors
    batch_nbr_fea_idx: paddle.Tensor shape (N, M)
      Indices of M neighbors of each atom
    crystal_atom_idx: list of paddle.Tensor of length N0
      Mapping from the crystal idx to atom idx
    target: paddle.Tensor shape (N, 1)
    ...
    return (batch_atom_fea, batch_nbr_fea, batch_nbr_fea_idx, crystal_atom_idx), \
